...
open class GetFavoriteCurrenciesUseCase @Inject constructor(private val currencyRepository: CurrenciesRepository,
                                                            @param:Named(SUBCRIBER_ON) private val subscriberOn: Scheduler,
                                                            @param:Named(OBSERVER_ON) private val observerOn: Scheduler
) :
    SingleUseCase<List<Currency>, GetFavoriteCurrenciesUseCase.Params>(subscriberOn, observerOn) {

    override fun buildUseCaseSingle(params: Params): Single<List<Currency>> {
        return currencyRepository.getFavorite()
    }
